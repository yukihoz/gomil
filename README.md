...
自分のリポジトリにforkまたはコピーしたうえで、任意の作業フォルダにcloneします。
...
1. GitHubで自分用の新しいリポジトリを作成します。
   例: `gomil`

2. このプロジェクトのファイルを自分のリポジトリへpushします。
...
すでにforkやcloneでGit管理されている場合は、`git init` と `git remote add origin ...` は不要です。通常どおり `git add`、`git commit`、`git push` してください。
